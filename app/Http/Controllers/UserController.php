...
{
    public function get_hris_information(Request $request){
        $test = SystemOneHris::where('EmpNo', $request->emp_no)->get();
        return $test;
        return response()->json(['test_user_data' => $test]);
    }
        ...
        if ($validator->passes()) {
            ...
            if (Auth::attempt($user_data)) {
                if (Auth::user()->is_password_changed == 0) {
                    ...
                    $_SESSION["rapidx_email"] = Auth::user()->email;
                    ...
                    /**
                     * Add laravel session to be used in laravel vue application
                     */
                    $request->session()->put('rapidx_user_id',  Auth::user()->id);
                    $request->session()->put('rapidx_user_level_id',  Auth::user()->user_level_id);
                    $request->session()->put('rapidx_username',  Auth::user()->username);
                    $request->session()->put('rapidx_name',  Auth::user()->name);
                    $request->session()->put('rapidx_department_id',  Auth::user()->department_id);
                    $request->session()->put('rapidx_employee_number',  Auth::user()->employee_number);
